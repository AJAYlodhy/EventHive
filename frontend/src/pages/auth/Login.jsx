...
import { Sparkles, ArrowRight, ShieldCheck, Mail, Lock, User, UserPlus } from 'lucide-react';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (mode === 'login') {
        ...
        await registerOrganizer({ name, email, password, organization });
      }
    ...
    <div className="min-h-screen flex items-center justify-center p-4 bg-gradient-to-br from-slate-950 via-slate-900 to-indigo-950/40 relative overflow-hidden">
      ...
      <div className="w-full max-w-md">
        ...
        <div className="rounded-3xl border border-slate-800 bg-slate-900/80 p-6 sm:p-8 backdrop-blur-xl shadow-2xl">
          ...
          {error && (
            <div className="mb-4 rounded-xl bg-rose-500/10 border border-rose-500/30 p-3 text-xs font-medium text-rose-300">
              {error}
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <>
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full rounded-xl border border-slate-800 bg-slate-950/60 py-2.5 px-4 text-sm text-white placeholder-slate-500 focus:border-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500"
                  />
                </div>
              </>
            )}
            ...
            <button
              type="submit"
              disabled={loading}
              className="mt-2 w-full flex items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-brand-600 to-indigo-600 py-3 text-sm font-bold text-white shadow-lg shadow-brand-600/30 hover:from-brand-500 hover:to-indigo-500 transition-all disabled:opacity-50"
            >
